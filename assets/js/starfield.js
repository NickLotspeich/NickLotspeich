...
//     const starsContainer = document.getElementById("skills");
...
document.addEventListener("DOMContentLoaded", function () {
    const starCount = 120; // Reduce the number of stars for simplicity
    ...
    // Position stars randomly within the "Skills" section
    const skillsRect = document.getElementById("skills").getBoundingClientRect();
    const stars = document.querySelectorAll(".star");
    stars.forEach(star => {
        const x = Math.random() * skillsRect.width;
        const y = Math.random() * skillsRect.height;
